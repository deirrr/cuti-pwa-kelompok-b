...
class PerbaruiUnitBisnisRequest extends FormRequest
{
    /**
    ...
    {
        /** @var UnitBisnis $unitBisnis */
        $unitBisnis = $this->route('unitBisnis');

        return [
            'kode' => ['required', 'string', 'max:20', 'regex:/^[A-Z0-9-]+$/', Rule::unique('unit_bisnis', 'kode')->ignore($unitBisnis)],
            'nama' => ['required', 'string', 'max:150', Rule::unique('unit_bisnis', 'nama')->ignore($unitBisnis)],
